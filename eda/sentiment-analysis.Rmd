...
below <- negative %>% 
  filter(average_star <= mean(reviews$stars))
percent_pos <- nrow(above) / nrow(positive)
percent_neg <- nrow(below) / nrow(negative)
print(str_c("Positivity Accuracy: ", percent_pos))
print(str_c("Negativity Accuracy: ", percent_neg))
```
64.23% of positive words lie above the mean average rating! There's definitely a positive correlation between word positivity in text reviews and Yelp business star ratings. Word negativity is even better at 87.24% accuracy of negative words lying below the mean average rating. 

What's the accuracy for words that are +5 or -5? Do they lead to 5-star ratings?
```{r}
pos_five <- summaries %>% filter(score == 5)
neg_five <- summaries %>% filter(score == -5)
```